...
def get_sites(previous_alexa, current_alexa, minimum_delta):
    lines = []
    ...
    for line in lines:
        ...
        if tokens[1] in delta:
            ...
            if tokens[0] - delta[tokens[1]] >= minimum_delta:
    ...
    with open('url_rank_drops', 'w') as f:
        f.write(output)

if __name__ == '__main__':
    if len(sys.argv) < 4:
        print 'Usage is python heuristic_url_drop_in_rank previous_alexa current_alexa minimum_delta'
    else:
        previous_alexa = sys.argv[1]
        current_alexa = sys.argv[2]
        minimum_delta = int(sys.argv[3])
        get_sites(previous_alexa, current_alexa, minimum_delta)
